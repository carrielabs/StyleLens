...
export default function ColorHighlighter({ src, targetHex }: { src: string, targetHex: string | null }) {
  ...
  useEffect(() => {
    ...
    if (minY !== Infinity) {
      // Find closest scrollable parent. In our layout, it's typically the enclosing div with .no-scrollbar
      const parentList = canvasRef.current?.closest('.no-scrollbar')
      if (parentList) {
        // The natural minY is in canvas space. Image is displayed with objectFit contain and max-width 500.
        // We roughly estimate the scroll position. It works reasonably well since the height scales linearly.
        const displayHeight = canvasRef.current.offsetHeight
        const scale = displayHeight / canvas.height
        const targetScrollTop = (minY * scale) - 100 // -100px padding
