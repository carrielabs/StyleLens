...
.ahp-dashboard-structure {
  display: grid;
  gap: 8px;
}
.ahp-dashboard-structure-group {
  display: grid;
  gap: 5px;
}
.ahp-dashboard-structure-group strong {
  color: rgba(255,255,255,0.72);
  font-size: 10px;
}
.ahp-dashboard-module-row {
  display: grid;
...
.ahp-dashboard-module-actions {
  display: flex;
  gap: 3px;
}
#ahp-style-panel .ahp-dashboard-module-actions button {
  height: 24px;
  min-width: 24px;
  padding: 0 6px;
  font-size: 10px;
...
(function () {
  ...
  function icon(name) {
    var icons = {
      ...
      undo: '<svg viewBox="0 0 24 24" fill="none" stroke-width="2"><path d="M9 14 4 9l5-5"/><path d="M4 9h10a6 6 0 0 1 0 12h-2"/></svg>',
      redo: '<svg viewBox="0 0 24 24" fill="none" stroke-width="2"><path d="m15 14 5-5-5-5"/><path d="M20 9H10a6 6 0 0 0 0 12h2"/></svg>',
  ...
  function cloneDashboardData(value) {
  ...
  function captureHistoryState(label) {
    return {
      ...
      dashboardData: cloneDashboardData(dashboardData)
    };
  }

  function sameHistoryState(left, right) {
    if (!left || !right) return false;
    return left.bodyHtml === right.bodyHtml
      && left.themeColor === right.themeColor
      && Boolean(left.themeTouched) === Boolean(right.themeTouched)
      && JSON.stringify(left.dashboardData || null) === JSON.stringify(right.dashboardData || null);
  }

  function updateHistoryControls() {
    if (!toolbar) return;
    var undoButton = toolbar.querySelector('[data-action="undo"]');
    var redoButton = toolbar.querySelector('[data-action="redo"]');
    var canUndo = historyIndex > 0;
    var canRedo = historyIndex >= 0 && historyIndex < historyStack.length - 1;
    if (undoButton) {
  ...
  function pushHistoryState(label) {
    ...
    var nextState = captureHistoryState(label);
    if (sameHistoryState(historyStack[historyIndex], nextState)) {
  ...
  function restoreHistoryState(state) {
    ...
    dashboardData = cloneDashboardData(state.dashboardData);
    if (dashboardData) window.AHP_DASHBOARD_DATA = dashboardData;
    else delete window.AHP_DASHBOARD_DATA;
    themeTouched = Boolean(state.themeTouched);
    themeColor = normalizeHexColor(state.themeColor) || initialThemeColor;
    restoreBodyHtml(state.bodyHtml, true);
    if (themeTouched) applyTheme(themeColor, true);
    else {
      clearThemeOverride();
      syncThemeControls(initialThemeColor);
    }
    if (dashboardData) {
      renderDashboardDataScript();
      applyDashboardStructureToPage();
    }
  ...
  function redoHistory() {
    ...
    historyIndex += 1;
    restoreHistoryState(historyStack[historyIndex]);
    if (status) status.textContent = '已重做';
  }
  ...
  function dashboardModules() {
    return dashboardData && Array.isArray(dashboardData.modules) ? dashboardData.modules : [];
  }

  function dashboardSections() {
    return dashboardData && Array.isArray(dashboardData.sections) ? dashboardData.sections : [];
  }

  function dashboardStructureSectionHtml() {
    var modules = dashboardModules();
    if (!modules.length) return '';
    var moduleById = {};
    modules.forEach(function (module) {
      moduleById[module.id] = module;
    });
    var groups = dashboardSections().map(function (section) {
      var rows = (section.moduleIds || []).map(function (moduleId) {
        var module = moduleById[moduleId];
        if (!module) return '';
        var visibleText = module.visible === false ? '显示' : '隐藏';
        return [
          '<div class="ahp-dashboard-module-row">',
          '<span title="' + htmlEscape(module.title) + '">' + htmlEscape(module.title) + '</span>',
          '<div class="ahp-dashboard-module-actions">',
          '<button type="button" data-dashboard-module-up="' + htmlEscape(module.id) + '" aria-label="上移 ' + htmlEscape(module.title) + '">上</button>',
          '<button type="button" data-dashboard-module-down="' + htmlEscape(module.id) + '" aria-label="下移 ' + htmlEscape(module.title) + '">下</button>',
          '<button type="button" data-dashboard-module-toggle="' + htmlEscape(module.id) + '">' + visibleText + '</button>',
          '</div>',
          '</div>'
        ].join('');
      }).join('');
      return '<div class="ahp-dashboard-structure-group"><strong>' + htmlEscape(section.title) + '</strong>' + rows + '</div>';
    }).join('');
    return [
      '<div class="ahp-panel-section" data-ahp-dashboard-structure="true"><div class="ahp-section-title">结构</div>',
      '<div class="ahp-dashboard-structure">',
      groups,
      '</div></div>'
    ].join('');
  }

  function dashboardModuleElement(moduleId) {
    return document.querySelector('[data-ahp-dashboard-module-id="' + moduleId + '"]');
  }

  function applyDashboardStructureToPage() {
    dashboardModules().forEach(function (module) {
      var element = dashboardModuleElement(module.id);
      if (!element) return;
      element.setAttribute('data-ahp-hidden', module.visible === false ? 'true' : 'false');
    });
  }

  function setDashboardModuleVisible(moduleId, visible) {
    dashboardModules().forEach(function (module) {
      if (module.id === moduleId) module.visible = visible;
    });
    applyDashboardStructureToPage();
    renderDashboardDataScript();
    saveCurrent();
    if (status) status.textContent = visible ? '模块已显示' : '模块已隐藏';
  }

  function toggleDashboardModule(moduleId, button) {
    var modules = dashboardModules();
    var module = modules.find(function (item) { return item.id === moduleId; });
    if (!module) return;
    var nextVisible = module.visible === false;
    setDashboardModuleVisible(moduleId, nextVisible);
    if (button) button.textContent = nextVisible ? '隐藏' : '显示';
  }

  function moveDashboardModule(moduleId, direction) {
    var element = dashboardModuleElement(moduleId);
    if (!element || !element.parentNode) return;
    var sibling = direction < 0 ? element.previousElementSibling : element.nextElementSibling;
    while (sibling && !sibling.matches('[data-ahp-dashboard-module-id]')) {
    ...
    reorderDashboardModuleData(moduleId, direction);
    renderDashboardDataScript();
    saveCurrent();
    if (status) status.textContent = '模块顺序已更新';
  }

  function reorderDashboardModuleData(moduleId, direction) {
    var modules = dashboardModules();
    var index = modules.findIndex(function (module) { return module.id === moduleId; });
    var nextIndex = index + direction;
    if (index < 0 || nextIndex < 0 || nextIndex >= modules.length) return;
    var moved = modules.splice(index, 1)[0];
    modules.splice(nextIndex, 0, moved);
    dashboardSections().forEach(function (section) {
      var ids = section.moduleIds || [];
      var sectionIndex = ids.indexOf(moduleId);
      var sectionNextIndex = sectionIndex + direction;
      if (sectionIndex < 0 || sectionNextIndex < 0 || sectionNextIndex >= ids.length) return;
      var movedId = ids.splice(sectionIndex, 1)[0];
      ids.splice(sectionNextIndex, 0, movedId);
    });
  }
  ...
  function createStylePanel() {
    ...
    stylePanel.addEventListener('click', function (event) {
      ...
      var dashboardToggle = event.target && event.target.closest ? event.target.closest('[data-dashboard-module-toggle]') : null;
      if (dashboardToggle) {
        toggleDashboardModule(dashboardToggle.getAttribute('data-dashboard-module-toggle'), dashboardToggle);
        return;
      }
      var dashboardUp = event.target && event.target.closest ? event.target.closest('[data-dashboard-module-up]') : null;
      if (dashboardUp) {
        moveDashboardModule(dashboardUp.getAttribute('data-dashboard-module-up'), -1);
        return;
      }
      var dashboardDown = event.target && event.target.closest ? event.target.closest('[data-dashboard-module-down]') : null;
      if (dashboardDown) {
        moveDashboardModule(dashboardDown.getAttribute('data-dashboard-module-down'), 1);
        return;
      }
  ...
  function createToolbar() {
    ...
    toolbar.innerHTML = [
      ...
      themeSwatchesHtml(),
      '</div></div>',
      '<div data-ahp-toolbar-section><span data-role="status" data-ahp-toolbar-status>可直接编辑</span><div data-ahp-toolbar-group><button type="button" data-action="undo" aria-label="撤销">' + icon('undo') + '<span>撤销</span></button>',
      '<button type="button" data-action="redo" aria-label="重做">' + icon('redo') + '<span>重做</span></button>',
      '<button type="button" data-action="save" aria-label="保存">' + icon('save') + '<span>保存</span></button>',
