...
export function generateCssVariables(report: StyleReport): string {
  ...
  const buttonPadding = pairPadding(button?.paddingV, button?.paddingH)
  const inputPadding = pairPadding(input?.paddingV, input?.paddingH)
  const cardPadding = card?.padding
...
:root {
  ...
  if (buttonPadding || inputPadding || cardPadding) {
    css += `\n\n  /* ── Components (DOM-measured snapshots) ───────── */`
    if (buttonPadding) css += `\n  --component-button-padding: ${buttonPadding};`
    if (button?.height) css += `\n  --component-button-height: ${button.height};`
    if (inputPadding) css += `\n  --component-input-padding: ${inputPadding};`
    if (cardPadding) css += `\n  --component-card-padding: ${cardPadding};`
  }
