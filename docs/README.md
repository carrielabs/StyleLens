...
StyleLens 是一个视觉风格提取工具：上传一张图或粘贴一个网址，自动生成包含色彩系统、字体排版、设计细节的风格报告，并可导出成 Prompt / Markdown / CSS / Design Token JSON。
...
这两份立项文档写得很细，视觉规范（色彩/字体/间距/圆角/阴影 token）目前仍然基本适用，**但技术选型部分已经和实际代码不一致**——最明显的一处：立项文档写"选 Claude 3.5 Sonnet 做 Vision 分析"，但实际代码（`lib/api/aiExtract.ts`）用的是 Google Gemini（`@google/generative-ai`），且专门做了国内网络访问的代理适配。这类落差在下面的 `guide/` 和 `roadmap.md` 里有说明，不在这份说明里重复。
